...
from .academic_year import current_school_year
from .models import Course, Curriculum, CurriculumCourse, DegreeLevel


class AcademicYearTestCase(TestCase):
    def test_current_school_year_uses_october_boundary(self):
        self.assertEqual(current_school_year(date(2026, 9, 30)), "2025/2026")
        self.assertEqual(current_school_year(date(2026, 10, 1)), "2026/2027")
...
class CurrentProfessorCourseApiTestCase(TestCase):
    def setUp(self):
        ...
        other_professor = ProfessorProfile.objects.create(user=other_user, employee_no="PROF-002")
